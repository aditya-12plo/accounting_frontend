            ...
            <div class="card card-docs mb-2">
              <!--begin::Card Body-->
                ...
                <div class="pb-10">
                  ...
                  <div class="py-5"><h1 class="anchor fw-bolder mb-5" id="overview" style="display: block;text-align: center;">ACCOUNTING & BUDGETING SYSTEM</h1> <br> {{ $t('txtLogin2') }}</div>
                  <div class="py-5">
                    <form class="pt-3" @submit.prevent="submitData" method="POST">
                        ...
                        <div class="form-group position-relative has-icon-left">
                            <label for="companyCode">{{ $t('companyCode') }}</label>
                            <div class="position-relative">
                                            
                                <v-select :options="companyModels" v-model="forms.company" :reduce="company => company.company_id" label="company_id">
                            ...
                            <div v-if="errors.company">
                                <div class="alert alert-light-success color-danger" v-for="error in errors.company" :key="error">{{error}}</div>
                            </div>
                        ...
                        <div class="my-2 d-flex justify-content-between align-items-center">
                            ...
                            <a :href="this.$settings.endPointApp+'/forgot-password'" class="auth-link text-black">{{ $t('forgotPassword') }}?</a>
                        </div>
...
export default {
  ...
  data () {
    return {
        ...
        companyModels:[],
        langs: ['id', 'en'],
        forms: {company:'', email:'', password: '', captchaCode:''},
    }
  },
   watch: { 
  ...
  methods: { 
        ...
        if (result.value) {
          ...
          if (this.forms.captchaCode == this.code) {
            ...
            if (this.forms.email.trim()) {
              ...
              const baseURI  =  this.$settings.endPoint+"/user/login";
              ...
              this.$http.post(baseURI,formData)
                .then((response) => {
                  ...
                  if(response.data.status === 200) {
                    ...
                    localStorage.setItem('companyCode', this.forms.company)
    ...
    fetchIt() {
        ...
        const baseURI  =  this.$settings.endPoint+"/get-companys";
            return this.$http.get(baseURI).then((response) => {
            this.companyModels = response.data.datas
            this.fade(false);
        })
    },
