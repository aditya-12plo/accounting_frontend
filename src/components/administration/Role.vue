            ...
            <div class="col-lg-12 col-xl-12 stretch-card">
              <div class="card">
                <div class="card-header">
                  <h4 style="margin-right: 5px" class="float-start">{{ $t("roleTxt") }}</h4>
                  <button
...
export default {
  ...
  methods: {
    ...
    downloadData() {
      const baseURI = this.$settings.endPoint + "/role";
      ...
      return this.$http
        .get(
          baseURI +
            `?per_page=${this.serverParams.per_page}&page=${this.serverParams.page}&sort_field=${this.serverParams.sort.field}&sort_type=${this.serverParams.sort.type}&name=${this.serverParams.columnFilters.name}&description=${this.serverParams.columnFilters.description}&download=download`,
          {
            responseType: "blob",
          }
        )
    ...
    uploadData(){
      window.location.href = "/role/upload";
    },

    editData(index, row) {
      var params = this.$onRandom(row.role_id);
    ...
    deleteData(index, row, status) {
      this.$swal({
        ...
        if (result.value) {
          ...
          const baseURI =
            this.$settings.endPoint + "role/delete/" + row.role_id;
          this.$http
    ...
    loadItems() {
      ...
      return this.$http
        .get(
           baseURI +
           `?per_page=${this.serverParams.per_page}&page=${this.serverParams.page}&sort_field=${this.serverParams.sort.field}&sort_type=${this.serverParams.sort.type}&name=${this.serverParams.columnFilters.name}&description=${this.serverParams.columnFilters.description}`,
        )
        .then((response) => {
