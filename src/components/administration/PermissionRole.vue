<template>
  <div>
    <div class="main-wrapper">
      <!--begin::sidebar-->
      <sidebar-component classMenu="PermissionRole"></sidebar-component>
      <!--end::sidebar-->
            ...
            <div class="col-lg-12 col-xl-12 stretch-card">
              <div class="card">
                <div class="card-header">
                  <h4 style="margin-right: 5px" class="float-start">Permission Role</h4>
                  <button
...
export default {
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          filterOptions: {
            enabled: true, // enable filter for this column
            placeholder: "Filter By Permission", // placeholder for filter input
            filterValue: "", // initial populated value for this filter
            filterDropdownItems: [], // dropdown (with selected values) instead of text input
            trigger: "enter", //only trigger on enter not on keyup
          },
  ...
  methods: {
    ...
    downloadData() {
      const baseURI = this.$settings.endPoint + "/permission-role";
      ...
      return this.$http
        .get(
          baseURI +
            `?per_page=${this.serverParams.per_page}&page=${this.serverParams.page}&sort_field=${this.serverParams.sort.field}&sort_type=${this.serverParams.sort.type}&code=${this.serverParams.columnFilters.code}&role=${this.serverParams.columnFilters.role}&permission=${this.serverParams.columnFilters.permission}&download=download`,
          {
            responseType: "blob",
          }
        )
    ...
    // },
    createData() {
      window.location.href = "/permission-role/add";
    },

    uploadData(){
      window.location.href = "/permission-role/upload";
    },

    editData(index, row) {
      var params = this.$onRandom(row.permission_role_id);
      window.location.href = "/permission-role/edit/" + params;
    },

    detailData(index, row) {
      var params = this.$onRandom(row.permission_role_id);
      window.location.href = "/permission-role/detail/" + params;
    },
    ...
    deleteData(index, row, status) {
      this.$swal({
        ...
        if (result.value) {
          ...
          const baseURI =
            this.$settings.endPoint + "permission-role/delete/" + row.permission_role_id;
          this.$http
    ...
    loadItems() {
      ...
      return this.$http
        .get(
           baseURI +
           `?per_page=${this.serverParams.per_page}&page=${this.serverParams.page}&sort_field=${this.serverParams.sort.field}&sort_type=${this.serverParams.sort.type}&code=${this.serverParams.columnFilters.code}&role=${this.serverParams.columnFilters.role}&permission=${this.serverParams.columnFilters.permission}`,
        )
        .then((response) => {
